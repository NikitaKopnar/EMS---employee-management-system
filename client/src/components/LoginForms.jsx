...
function LoginForms({ role, title, subtitle }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.respons?.data?.error || error.message || "Login failed",
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="w-full max-w-md animate-fade-in">
          ...
          {error && (
            <div className="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-700 text-sm rounded-xl flex items-start gap-3">
              <div className="w-1.5 h-1.5 rounded-full bg-rose-500 mg-1.5 shrink-0" />
              {error}
            </div>
          )}
